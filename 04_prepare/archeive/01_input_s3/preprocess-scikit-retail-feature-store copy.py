...
def process(args):
    ...
    # 3) หา input CSV ใต้ input-data
    input_files = glob.glob(os.path.join(args.input_data, "*.csv"))
    print("Input files found: {}".format(input_files))

    if not input_files:
        raise FileNotFoundError("No CSV files found under {}".format(args.input_data))

    # 4) ทำ feature engineering + ingest สำหรับทุกไฟล์ (ส่วนมากของเรามีไฟล์เดียว)
    ...
    # 5) รอให้ offline store มีไฟล์จริง ๆ
